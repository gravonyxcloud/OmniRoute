...
test("a stale release branch skips without overwriting next", () => {
  assert.equal(resolveVersion("push", "branch", "release/v3.8.49", "", "release/v3.8.50"), "skip");
...
test("fork convention: every versioned publish promotes :latest", () => {
  // The workflow should promote :latest for any published version (not skip/main/next).
  // This replaces the old semver-gated logic (should-promote-latest.sh) for the fork.
  assert.match(WORKFLOW, /Fork convention: every published versioned build moves :latest/);
  // The decision block must set PROMOTE=true for tags and release events, not call should-promote-latest.sh
  assert.doesNotMatch(WORKFLOW, /bash scripts\/ci\/should-promote-latest\.sh/);
  assert.match(WORKFLOW, /PROMOTE="true"/);
});
